(ns pail-graph.structure-test
  (:require [pail-graph.fakes.structure :as fakes]
            ...
            [pail-graph-tap.tapmapper :as tapmapper])
  (:import [com.backtype.hadoop.pail PailStructure]
           [clj_pail.partitioner NullPartitioner]
           [clj_pail_tap.fakes.structure DefaultPailStructure FakePailStructure FakeSerializer FakePartitioner UnserializableStateStructure]
...
(facts "gen-structure"
  (facts "default structure"
    (let [default-structure (DefaultPailStructure.)]
      ...
      (fact "getPropertyPathGenerator returns path generator function"
        (.getPropertyPathGenerator default-structure) => tapmapper/nullpropertypathgenerator)

      (fact "PropertyPathGenerator returns empty set by default."
            ((.getPropertyPathGenerator default-structure) ["some" "path"]) => {})
